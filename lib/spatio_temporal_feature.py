...
class TimeSurface(object):
    ...
    def _update_time_context(self, event):
        ...
        x, y = event.x, event.y
        self.time_context = self.latest_times[y - self.r:y + self.r + 1, x - self.r:x + self.r + 1]     # 取范围

    def _update_time_surface(self, current_time):
        """ create time surface from time context """

        self.time_surface = np.exp(-1. * (current_time - self.time_context) / self.time_constant)       # 公式算surface
