...
def visualise_time_surface_for_event_stream(N, tau, r, width, height, events):
    ...
    ts_1_1 = TimeSurface(height, width, region_size=r, time_constant=tau)       # 正极性，亮度增加
    ts_1_2 = TimeSurface(height, width, region_size=r, time_constant=tau)       # 负极性，亮度减少
    ...
    event_data = events
    event_data_filt, _, _ = remove_isolated_pixels(event_data, eps=5, min_samples=10)

    for e in event_data_filt:       # 每一个事件（像素点坐标、时间戳、极性）
        if e.ts <= t_pause:
            if e.p:
    ...
    if True:
        ...
        ax[0, 0].imshow(ts_1_1.latest_times)                # 事件流的宏观结果
        ax[0, 1].imshow(ts_1_1.time_context)                # 最后一个正事件的内容（最后一个变亮像素的区块）
        ax[0, 2].imshow(ts_1_1.time_surface, vmin=0, vmax=1)    # 最后一个正事件的surface（由内容计算而得）
...
def initialise_time_surface_prototypes(N, tau, r, width, height, events, init_method=6, plot=False):    # TODO: set default init method to -1
    ...
    if init_method == 1:
        ...
        for i in range(N):
            C[i] = np.random.choice([0, 1], size=(C[i].shape[0], C[i].shape[1]), p=[0.995, 0.005]).astype(np.float64)
...
def train_layer(C, N, tau, r, width, height, events, num_polarities, layer_number, plot=False):
    ...
    p = [1] * N             # 每个原型的计数器，记录每个原型被匹配的次数
    alpha_hist = []         # 学习率的历史记录
    beta_hist = []          # 余弦相似度的历史记录
    p_hist = []             # 每个原型的计数器的历史记录
    k_hist = []             # 每次匹配的原型索引的历史记录
    dists_hist = []         # 每次计算的距离的历史记录
    S_prev = deepcopy(S)    # 上一次的时间曲面状态
    event_times = []        # 事件的时间戳记录

    for i, e in enumerate(events):
        valid = S[e.p].process_event(e)  # 处理事件 e，返回是否有效

        if not valid:
            continue  # 如果事件无效，则跳过

        # 找到最近的聚类中心（即最近的时间曲面原型，根据欧几里得距离）
        dists = [euclidean_dist(c_k.reshape(-1), S[e.p].time_surface.reshape(-1)) for c_k in C]
        k = np.argmin(dists)  # 找到最近的原型索引

        # 更新最近的原型
        alpha = 0.01 / (1 + p[k] / 20000.)  # 学习率，随着原型被匹配次数的增加而减小
        beta = cosine_dist(C[k].reshape(-1), S[e.p].time_surface.reshape(-1))  # 余弦相似度
        C[k] += alpha * (S[e.p].time_surface - beta * C[k])  # 更新原型
        p[k] += 1  # 更新原型的计数器
