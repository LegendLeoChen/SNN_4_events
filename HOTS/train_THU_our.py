...
def event_convert(event, width, height):
    # 创建 Events 对象
    ev = eventvision.Events(event.shape[0], 640, 480)
    # 提取时间戳并减去最小值
    timestamps = (event[:, 2] * 10000).astype(np.int64)
    min_timestamp = timestamps.min()
    normalized_timestamps = (timestamps - min_timestamp).astype(np.int32)
    # 赋值给 ev.data
    ev.data.x = event[:, 0]
    ev.data.y = event[:, 1]
    ev.data.ts = normalized_timestamps  # 使用归一化后的时间戳
    ev.data.p = event[:, 3]
    # 设置宽度和高度
    ev.width = width
    ev.height = height
...
def main():
    ...
    # data_directory = "./datasets/THU-EACT-50-CHL"
    data_directory = "./datasets/test"
    repr = ['timeSurface']
    dataset = THU_EACT_50_CHL(datafile=data_directory, eval=True, augmentation=False, repr=repr)

    index_to_test = 0                                           # 选取的样本编号
    ev = dataset.__getitem__(index_to_test, True, False)        # 获取数据
    ev = event_convert(ev, width, height)                       # 转为event

    print("单个事件流数据形状:", ev.data.shape)
    print("数据集长度", len(dataset))

    input_files_all = []                # 所有事件流

    for i in range(1):                 # 取若干事件流训练
        input_files_all.extend([dataset.__getitem__(index_to_test, True, False)])

    # 初始化事件数据列表
    event_data = []
    event_data_filt = []

    # 遍历所有事件流合并为长事件流（每个文件是一段事件流）
    for ev_data in input_files_all:
        # 读取事件数据
        ev_data = event_convert(ev_data, width, height).data
        # ev_data = eventvision.read_dataset(f).data
        # 去除孤立像素
        ev_data_filt = remove_isolated_pixels(ev_data, eps=5, min_samples=10)[0]
        # 确保事件流的时间戳是单调递增的
        if len(event_data) > 0:
    ...
    # 训练第一层的时间曲面原型
    C_1 = initialise_time_surface_prototypes(N_1, tau_1, r_1, ev.width, ev.height, event_data_filt, init_method=8, plot=True)  # 初始化（随机）
    ...
    C_2 = initialise_time_surface_prototypes(N_2, tau_2, r_2, ev.width, ev.height, event_data_2, init_method=8, plot=True)
    ...
    C_3 = initialise_time_surface_prototypes(N_3, tau_3, r_3, ev.width, ev.height, event_data_3, init_method=8, plot=True)

    train_layer(C_3, N_3, tau_3, r_3, ev.width, ev.height, event_data_3, num_polarities=N_2, layer_number=3, plot=True)

    ev.data = remove_isolated_pixels(ev.data, eps=5, min_samples=10)[0]
