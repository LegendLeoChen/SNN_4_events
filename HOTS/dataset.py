...
class THU_EACT_50_CHL:
    ...
    def __getitem__(self, idx, events_only=False, normalize=False):
        """
        returns events and label, loading events from aedat
        :param idx:
        :return: x,y,t,p,  label
        ...
        events = np.load(f).astype(np.float64)

        if normalize:
            # normalize the timestamps 归一化时间戳
            _min = events[:,2].min()
            _max = events[:,2].max()
            events[:,2] = (events[:,2] - _min) / (_max - _min)
        ...
        for repr_name in self.repr:
            repr_array = repr_map[repr_name](events[:, 2], events[:, 0].astype(np.int32), events[:, 1].astype(np.int32),
                                             events[:, 3],
                                             repr_size=(480, 640), time_num=self.time_num)
...
if __name__ == '__main__':
    ...
    index_to_test = 5  # index of the sample you want to test
    # single_sample_reprs, single_sample_label = dataset.__getitem__(index_to_test, True)
    single_sample_reprs = dataset.__getitem__(index_to_test, True, True)
    # Output the results
